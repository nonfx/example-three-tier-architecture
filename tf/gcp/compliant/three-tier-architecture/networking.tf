...
resource "google_compute_backend_service" "compliant_backend" {
  ...
  # Updated logging configuration to enable logging and set sample rate to 1.0
  # This change ensures compliance with CIS benchmark 2.16 which requires logging to be enabled
  # for HTTP(S) Load Balancer backend services with a sample rate greater than 0
  log_config {
    enable      = true
    sample_rate = 1.0
  }
